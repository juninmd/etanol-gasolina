...
const CelebrationContent = observer(({stationsStore}: Props) => {
  const [particles] = useState(() =>
    Array.from({length: PARTICLE_COUNT}).map(() => ({
      ...
      color: ['#FFD700', '#FF4500', '#00FA9A', '#1E90FF'][
  ...
  }, [particles, scaleAnim]); // Run once on mount
        ...
        <Card style={styles.card} status="success">
          {isBadge ? (
            <View style={{alignItems: 'center'}}>
              <Icon
                name={stationsStore.currentBadge!.icon}
                width={60}
                height={60}
                fill="#FFD700"
            ...
            <View style={{alignItems: 'center'}}>
              <Text category="h1" style={styles.emoji}>
                🎉
              </Text>
              <Text category="h3" style={styles.title}>
                LEVEL UP!
...
const Celebration = inject('stationsStore')(
  observer(({stationsStore}: Props) => {
    const showCelebration =
      stationsStore?.showLevelUp || !!stationsStore?.currentBadge;
