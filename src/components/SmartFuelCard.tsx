...
import { Card, Text, Icon, Button, Layout, useTheme } from '@ui-kitten/components';
...
const SmartFuelCard = observer(({ stationsStore }: Props) => {
    ...
    const { avgGas, avgEthanol, ratio, bestFuel, potentialSavingsPct } = marketAnalysis;
         ...
         return (
            <Card style={styles.card}>
                <View style={styles.header}>
                     <View style={[styles.iconContainer, { backgroundColor: theme['color-basic-200'] }]}>
                        <Icon name='activity-outline' width={24} height={24} fill={theme['text-hint-color']} />
                    </View>
                    <View style={styles.headerText}>
                        <Text category='h6'>Analisando Mercado...</Text>
                        <Text category='c1' appearance='hint'>Aguarde enquanto buscamos dados</Text>
                    </View>
                </View>
            </Card>
         );
    ...
    const fillColor = isEthanol ? theme['color-success-500'] : theme['color-info-500'];
    ...
    return (
        <Card status={status} style={styles.card}>
            <View style={styles.header}>
                <View style={[styles.iconContainer, { backgroundColor: isEthanol ? theme['color-success-100'] : theme['color-info-100'] }]}>
                    <Icon
                ...
                <View style={styles.headerText}>
                    ...
                    <Text category='c1' appearance='hint'>Análise de mercado em tempo real</Text>
                </View>
            </View>
            ...
            <View style={styles.mainContent}>
                ...
                {potentialSavingsPct > 0 && (
                    <Text category='s1' style={{ marginTop: 5 }}>
                        Economia estimada de <Text style={{fontWeight: 'bold', color: fillColor}}>{potentialSavingsPct}%</Text>
                    </Text>
                )}
            </View>
            ...
            <View style={styles.gaugeContainer}>
                ...
                <View style={styles.barContainer}>
                    ...
                    <View style={[styles.zone, { width: `${pivotPos}%`, backgroundColor: theme['color-success-200'] }]} />
                    <View style={[styles.zone, { width: `${100 - pivotPos}%`, backgroundColor: theme['color-info-200'] }]} />

                    {/* Pivot Line (0.7) */}
                    <View style={[styles.pivotLine, { left: `${pivotPos}%` }]} />

                    {/* Current Position Marker */}
                    <View style={[styles.marker, { left: `${positionPct}%`, backgroundColor: theme['text-basic-color'] }]}>
            ...
            <View style={styles.footer}>
                <View style={styles.priceTag}>
                    <Text category='c2' appearance='hint'>Média Etanol</Text>
                    <Text category='s2'>R$ {avgEthanol.toFixed(2)}</Text>
                </View>
                <View style={styles.divider} />
                <View style={styles.priceTag}>
                    <Text category='c2' appearance='hint'>Média Gasolina</Text>
