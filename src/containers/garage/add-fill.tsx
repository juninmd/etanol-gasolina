...
export default class AddFill extends Component<Props, State> {
  constructor(props: Props) {
    ...
    const station = stationId
      ? stationsStore.stations.find((s) => s.id === stationId)
      : null;
  ...
  handleScanReceipt = () => {
    ...
    this.scanTimeout = setTimeout(() => {
      ...
      this.successTimeout = setTimeout(() => {
        ...
        this.props.stationsStore.triggerAlert(
          '✨ Nota Fiscal lida com sucesso via IA!',
          'success',
        );
      }, 1000);
  ...
  handleSave = () => {
    ...
    if (stationName && price && liters) {
      ...
      // Calculate Savings
      try {
        let station = route.params?.stationId
          ? stationsStore.stations.find((s) => s.id === route.params.stationId)
          : null;
        if (!station) {
          station = stationsStore.stations.find(
        ...
        if (station && vehicle) {
          ...
          if (savings > 0) {
            ...
            alert(
              `Economia registrada! Você economizou R$ ${savings.toFixed(2)}.`,
            );
          }
        }
      } catch (e) {
    ...
    return (
      <Layout style={styles.container}>
        ...
        <Modal
          ...
          onBackdropPress={() =>
            !this.state.isScanning && this.setState({showScannerModal: false})
          }>
