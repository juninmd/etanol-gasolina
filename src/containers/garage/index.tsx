...
export default class Garage extends Component<Props> {
      ...
      <Card
        ...
        <View style={styles.vehicleHeader}>
          <Icon
            ...
            width={32}
            height={32}
            fill={isSelected ? '#fff' : '#3366FF'}
          />
          <View style={styles.vehicleInfo}>
            <Text category="h6" style={isSelected ? styles.whiteText : null}>
              {item.name}
            </Text>
            <Text
              category="s2"
              style={isSelected ? styles.whiteText : styles.subText}>
              Gas: {item.avgGasConsumption} km/l | Eta:{' '}
              {item.avgEthanolConsumption} km/l
            </Text>
          </View>
          {isSelected && (
            <Icon
  ...
  renderLogItem = ({item}) => (
    <ListItem
      ...
      description={`${item.liters.toFixed(2)}L de ${
        item.fuelType === 'gas' ? 'Gasolina' : 'Etanol'
      } - R$ ${item.totalCost.toFixed(2)}`}
      accessoryRight={() => (
        <Text status={item.fuelType === 'gas' ? 'info' : 'success'}>
          {item.fuelType === 'gas' ? 'GAS' : 'ETH'}
        </Text>
      )}
    />
  );
          ...
          {selectedVehicle ? (
            <>
              <Card style={styles.statsCard} status="primary">
                <Text category="h6" style={styles.whiteText}>
                  Resumo de Gastos
                </Text>
                <Text category="h1" style={styles.whiteText}>
                ...
                <View style={styles.row}>
                  <View>
                    ...
                    <Text category="h6" style={styles.whiteText}>
                      {fuelStats.totalGasLiters.toFixed(1)} L
                    </Text>
                  </View>
                  <View>
                    <Text style={styles.whiteText}>Etanol</Text>
                    <Text category="h6" style={styles.whiteText}>
                      {fuelStats.totalEthLiters.toFixed(1)} L
                    </Text>
                  </View>
                </View>
              </Card>
                ...
                logsForSelectedVehicle.map(log => (
                  <View key={log.id}>
                    <ListItem
                      ...
                      description={`${log.liters.toFixed(2)}L de ${
                        log.fuelType === 'gas' ? 'Gasolina' : 'Etanol'
                      } - R$ ${log.totalCost.toFixed(2)}`}
                      accessoryRight={() => (
                        <Text
                          category="s2"
                          status={log.fuelType === 'gas' ? 'info' : 'success'}>
                          {log.fuelType === 'gas' ? 'GAS' : 'ETH'}
                        </Text>
                      )}
                    />
                    <Divider />
            ...
            <Text style={{textAlign: 'center', marginTop: 50}}>
              Selecione ou adicione um veículo.
            </Text>
          )}
        </ScrollView>
      </Layout>
