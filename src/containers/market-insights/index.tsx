...
export default class MarketInsights extends Component<Props> {
  ...
  get aggregatedHistory() {
    ...
    // Add today's average
    const todayAvg =
      stations.reduce((acc, s) => acc + s.priceGas, 0) / stations.length;
    history.push({
      date: 'Hoje',
      avg: todayAvg,
    });
  ...
  renderChart = () => {
    ...
    if (data.length === 0) {
      return <Text>Sem dados suficientes.</Text>;
    }
          ...
          return (
            <View key={index} style={styles.chartBarContainer}>
              <View style={styles.barLabelContainer}>
                <Text category="c1" style={styles.barValue}>
              ...
              <View
                style={[
                  ...
                  index === data.length - 1 ? styles.activeBar : null,
                ]}
              />
              <Text category="c2" style={styles.barLabel}>
                {item.date}
              </Text>
            </View>
          );
      ...
      <Card style={[styles.sentimentCard, {borderTopColor: color}]}>
        <View style={styles.sentimentHeader}>
          ...
          <View style={{marginLeft: 15}}>
            <Text category="h5" style={{color: color, fontWeight: 'bold'}}>
              {text}
            </Text>
        ...
        <View style={styles.predictionBox}>
          ...
          <View
            ...
            <Text category="s2" style={{marginLeft: 5}}>
              {sentiment === 'buy'
                ? 'Probabilidade de aumento em 48h.'
                : 'Os preços devem estabilizar em breve.'}
            </Text>
          </View>
        </View>
      </Card>
          ...
          return (
            <View key={index} style={styles.chartBarContainer}>
              <View style={styles.barLabelContainer}>
                <Text
                  category="c1"
                  style={[styles.barValue, {color: '#00E096'}]}>
            ...
            <View style={styles.statsRow}>
              ...
              <Card style={styles.statCard}>
                ...
                <Text category="h4" status="info">
                  R${' '}
                  {(
                    this.aggregatedHistory[this.aggregatedHistory.length - 1]
                      ?.avg || 0
                  ).toFixed(2)}
                </Text>
            ...
            <Card style={styles.tipCard} status="warning">
              <View style={{flexDirection: 'row'}}>
                ...
                <Text style={styles.tipText}>
                  Dica: Abastecer no{' '}
                  {this.props.stationsStore?.bestStation?.name} agora
                  economizaria aproximadamente R${' '}
                  {(
                    (this.aggregatedHistory[this.aggregatedHistory.length - 1]
                      ?.avg || 0) -
                    (this.props.stationsStore?.bestStation?.priceGas || 0)
                  ).toFixed(2)}{' '}
                  por litro em comparação à média.
                </Text>
              </View>
            </Card>
          </Animated.View>
