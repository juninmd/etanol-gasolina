...
export default class StationDetails extends Component<Props, State> {
  ...
  renderRightAction = () => {
    ...
    return (
      <TopNavigationAction
        icon={HeartIcon}
        onPress={() => stationsStore.toggleFavorite(stationId)}
      />
    );
  };
  ...
  handleUpdatePrice = () => {
    ...
    if (newGasPrice && newEthanolPrice) {
      stationsStore.updatePrice(
        stationId,
        parseFloat(newGasPrice),
        parseFloat(newEthanolPrice),
      );
      this.setState({
        showUpdatePrice: false,
        newGasPrice: '',
  ...
  render() {
    ...
    // Reliability Logic
    const lastVerifiedDate = station.lastVerified
      ? new Date(station.lastVerified)
      : new Date(0);
    const hoursSince =
      (new Date().getTime() - lastVerifiedDate.getTime()) / (1000 * 60 * 60);
        ...
        <ScrollView contentContainerStyle={styles.content}>
          <Card style={styles.card}>
            <View
              ...
              <View style={{flex: 1}}>
                ...
                <Text category="s1" style={styles.address}>
                  {station.address}
                </Text>
              </View>
              {/* Live Badge */}
              <Animated.View
                style={[styles.liveBadge, {opacity: this.pulseAnim}]}>
            ...
            <View style={[styles.trustBadge, {borderColor: trustColor}]}>
              <Icon
                name={hoursSince < 24 ? 'checkmark-circle-2' : 'alert-circle'}
                width={16}
                height={16}
                fill={trustColor}
              />
              <Text
                category="c1"
                style={{color: trustColor, marginLeft: 5, fontWeight: 'bold'}}>
                {trustText}
              </Text>
              <Text
                category="c2"
                ...
                {station.verificationsCount || 0} verificações
              </Text>
            </View>
            ...
            <View style={styles.priceRow}>
              <View style={styles.priceBox}>
                ...
                <Text category="h4" status="info">
                  R$ {station.priceGas.toFixed(2)}
                </Text>
              </View>
              <View style={styles.priceBox}>
                <Text category="label">Etanol</Text>
                <Text category="h4" status="success">
                  R$ {station.priceEthanol.toFixed(2)}
                </Text>
              </View>
            </View>
            {station.isPromo && (
              <Button status="warning" size="small" style={styles.promoBadge}>
                Em Promoção!
              </Button>
            )}
          </Card>
          ...
          <Card
            style={[
              ...
              {
                backgroundColor:
                  station.priceEthanol / station.priceGas < 0.7
                    ? '#F0FFF4'
                    : '#FFF5F5',
              },
            ]}>
            <View
              ...
              <View>
                ...
                <Text category="c2" appearance="hint">
                  ...
                  {((station.priceEthanol / station.priceGas) * 100).toFixed(0)}
                  %
                </Text>
              </View>
          ...
          <Card
            ...
            <Text category="p2">
              ...
              .
              {station.priceTrend === 'down'
                ? ' Pode valer a pena esperar um pouco!'
                : station.priceTrend === 'up'
                ? ' Recomendamos abastecer agora para garantir este preço.'
          ...
          <Card style={styles.card}>
            ...
            <Text category="c2" appearance="hint" style={{marginBottom: 10}}>
              Veja quanto rende o seu dinheiro aqui.
            </Text>
            <Input
            ...
            {this.state.simAmount ? (
              <View
                ...
                <View style={{alignItems: 'center'}}>
                  ...
                  <Text category="h4" status="info">
                    {(
                      parseFloat(this.state.simAmount.replace(',', '.')) /
                      station.priceGas
                    ).toFixed(2)}{' '}
                    L
                  </Text>
